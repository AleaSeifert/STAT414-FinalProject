...
```{r}
set.seed(7000)
baseball_sample <- df_baseball_clean
...
random_effects <- ranef(model6)$batter

random_effects_df <- as.data.frame(random_effects) |>
  mutate(batter = rownames(random_effects)) |>
  rename(random_intercept = `(Intercept)`, random_slope = `launch_angle_gmc`)

ggplot(model6, aes(x = launch_angle_gmc, y = hit_distance_sc, group=batter)) +
  geom_line(aes(y = predict(model6, re.form = ~ (1 + launch_angle_gmc | batter))), alpha = .5, color = "blue") +
  geom_abline(intercept = fixef(model6)["(Intercept)"],
              slope = fixef(model6)["launch_angle_gmc"],
              color = "black", size = 1.2) +
  labs(title = "Random Effects Overlaid on Fixed Effects",
       x = "Launch Angle (GMC)",
