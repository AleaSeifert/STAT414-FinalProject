...
model1 <- lmer(hit_distance_sc ~ launch_angle_gmc + launch_speed_gmc + is_fastball + (1 | batter), data=final_data)
summary(model1)
...
model4 <- lmer(hit_distance_sc ~ launch_angle_gmc + launch_speed_gmc + (1 | is_fastball) + (1 | batter), data=final_data)
performance(model4)
```

**In model 1, we added our three level 1 variables of interest (launch speed, launch angles, and if its a fastball) and used a likelihood ratio test to compare. Using an anova to compare the null model and model 1, the likelihood test gave us a large chi-square test statistic of 6488.3 and a p-value less than 0.001. The likelihood test had 3 degrees of freedom, which is the difference in the number of parameters between the null model and model 1. The AIC of model 1 is 119931 which is less than the null model AIC of 126413. Model 1 also has a BIC of 119974 which is less than the null models BIC of 126434. Therefore there is strong evidence to conclude that model 1 with all the level 1 variables is a better fit for the data than the random intercepts null model.**

$$Total variation in null model = {51.93 + 9414.27} = 9466.2$$ 

$$Total variation in model 1 = {278.2 + 17917.6} = 18195.8$$

$$\frac{18195.8 - 9466.2}{18195.8} = 0.4798$$

**The level 1 variables is_fastball, launch angle, and launch speed explain 47.98% of the variation in hit distance. Yes, the level 2 variance decreased from _ to _.**

**We then created model 2 that includes launch speed, launch angle, and batter random intercepts. We also created model 3 which only includes launch angle and batter random intercepts. We then ran an anova of models 1, 2, and 3. We found that model 2, is a better fit of the data than model 3 and launch speed is a statistically significant predictor of hit distance (chi-square = 4954.6, p-value < 0.001). However, we found that model 1 was not significantly better fit of the data than model 2 and the variable is_fastball is not a statistically significant predictor of hit distance (chi-square = 3.7194, p-value = 0.05378). ##Since model 2 (includes launch speed, launch angle, and batter random intercepts) is simpler than model 1, we think it is the best model so far. (maybe delete)**
...
model5 <- lmer(hit_distance_sc ~ launch_angle_gmc + launch_speed_gmc + is_fastball + stand + (1 | batter), data = final_data)
...
**Model 5 includes three level 1 variables and where the batter stands as level 2 variable. Model 5 has an AIC of 119918 which is larger than the AIC for model 1 which is 119880.**
